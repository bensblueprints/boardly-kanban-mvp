...
3. On a supported computer, select **Install Qwen · about 6 GB**. Setup needs Linux x86_64, Python 3.11+, curl, a user systemd session and Vulkan graphics drivers. A GPU with 16 GB of VRAM is recommended; the inference test is the compatibility check. Native Windows/macOS installation is not included in this installer.
...
Connections, successful tests and selected modes belong to the authenticated account workspace. A different account cannot select your connection ID. Members need both desktop permission and access to the selected account SSH connection; selecting local vision does not grant them shell access or override existing sharing. Revocation is checked while inference is running. Human takeover and desktop leases remain enforced, including a lease renewal during longer inference.
...
The native helper's `inspect({desktop_id,question})` uses the existing screenshot broker command. This lets a running worker adopt local text results without interrupting its active tasks. Update its helper file before enabling local mode; the matching worker image contains the updated planner instructions for subsequent process starts. Existing workers learn the current mode and focused-question behavior from computer listing and observation results. Do not restart active Codex processes merely to refresh these instructions.
