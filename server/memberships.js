...
function createMemberships(root){
 ...
 if(!db.prepare('PRAGMA table_info(account_members)').all().some(c=>c.name==='owner_ssh'))db.exec('ALTER TABLE account_members ADD COLUMN owner_ssh INTEGER NOT NULL DEFAULT 0');
 ...
 const grant=id=>{const row=db.prepare('SELECT g.*,m.owner_id,m.user_id,m.email,m.owner_ssh FROM access_grants g JOIN account_members m ON m.id=g.member_id WHERE g.id=?').get(id);return row?{...row,scopes:readScopes(row.scopes)}:null;};
 ...
 function grants(ownerId,userId){return db.prepare('SELECT g.*,m.owner_ssh FROM access_grants g JOIN account_members m ON m.id=g.member_id WHERE m.owner_id=? AND m.user_id=?').all(ownerId,userId).map(g=>({...g,scopes:readScopes(g.scopes)}));}
 function list(ownerId,kind,resourceId){return db.prepare('SELECT m.id,m.email,m.name,m.status,m.last_seen_at,m.owner_ssh,g.id AS grant_id,g.kind,g.resource_id,g.role,g.scopes,g.scope_company_id FROM access_grants g JOIN account_members m ON m.id=g.member_id WHERE m.owner_id=? AND g.kind=? AND g.resource_id=? ORDER BY m.email').all(ownerId,kind,resourceId).map(g=>({...g,scopes:readScopes(g.scopes)}));}
 ...
 function setOwnerSsh(ownerId,id,value,actorId){
  if(actorId!==ownerId)throw fail(403,'Only the account owner can change shared SSH permission');
  if(typeof value!=='boolean')throw fail(400,'Shared SSH permission must be on or off');
  return db.transaction(()=>{const g=grant(id);if(!g||g.owner_id!==ownerId)throw fail(404,'Membership not found');
   db.prepare('UPDATE account_members SET owner_ssh=? WHERE id=? AND owner_id=?').run(Number(value),g.member_id,ownerId);
   db.prepare('INSERT INTO member_scope_events VALUES (?,?,?,?,?,?,?)').run(crypto.randomUUID(),ownerId,actorId,id,JSON.stringify({owner_ssh:!!g.owner_ssh}),JSON.stringify({owner_ssh:value}),Date.now());
  })();
 }
 ...
 return{db,usage,prune,add,grants,grant,list,update,setScopes,setOwnerSsh,remove,accounts,
