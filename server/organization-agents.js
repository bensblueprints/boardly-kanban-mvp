...
function createOrganizationAgents({db, clean, userId, hosted}) {
  ...
  router.get('/api/agents/:kind(company|board)/:id', (req,res) => {
    ...
    res.json({scope:s,swarms,threads,max_agents:MAX_AGENTS});
  });
  ...
  function context(j){const t=getThread(j.thread_id),s=scope(t.scope_type,t.scope_id);return {kind:'discussion',...j,context:cleanValues({scope:s,projects:snapshot(db,s.projects.map(p=>p.id))},value=>scrub(s,value)),history:db.prepare("SELECT mode,prompt,draft,status FROM discussion_jobs WHERE thread_id=? AND created_at<=? ORDER BY created_at,rowid").all(t.id,j.created_at).slice(-20)};}
