 ...
 async function run(j){
  ...
  try{
   ...
   for(let step=Number(j.continuation_count)||0;!closed;step++){
    ...
    const response=await retryResponse(j,allowed,progress,()=>personal.respond(a,j.id,{model:a.model,service_tier:'default',store:false,input:input.map(({boardly_connection_context,...item})=>item),tools:j.mode==='work'?tools.filter(tool=>toolAllowed(tool.name)):[],max_output_tokens:4096}));
