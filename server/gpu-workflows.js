...
function createGpuWorkflows({db, ssh, ownerId, generate, retain = () => {}, release = () => {}, remote, interval = 15000}) {
  ...
  function saveWorkflow(input,id) {
    ...
    const steps = input.steps.map(s => {
      if (!['prompt','image','video'].includes(s.kind)) throw fail(400,'Choose an AI prompt, image or video step.');
  ...
  function start(id,input) {
    ...
    const steps = definition.steps.map(s => ({...s,id:crypto.randomUUID(),status:'queued',...(s.template_id ? {template:ts.find(t => t.id === s.template_id)} : {})}));
  ...
  async function advance(r) {
    ...
    const step = r.steps[r.step_index]; if (!step) {r.status='completed'; update(r); return;}
    ...
    try {
      ...
      const current = run(r.id); if (['paused','cancelled'].includes(current.status)) r.status=current.status;
  ...
  router.get('/api/gpu/dashboard',(req,res)=>{
    ...
    res.json({workers:workers(),templates:templates().map(({graph,...t})=>t),workflows:workflows(),runs});
  });
  ...
  router.post('/api/gpu/workflows/:id/run',(req,res)=>res.status(202).json(start(req.params.id,req.body)));
  router.post('/api/gpu/runs/:id/:action(pause|resume|retry|cancel)',(req,res)=>{
    ...
    if (action==='resume' && r.status==='blocked') throw fail(409,'Resolve the failed step before retrying.');
  ...
  router.get('/api/gpu/workers/:id/output',async(req,res,next)=>{
    try {
      ...
      await ssh.execute(connection,{forward:{host:'127.0.0.1',port},requireEnabled:true,valid,consumeForward:(sock,stillValid)=>new Promise((resolve,reject)=>{
        ...
        let check;const finish=error=>{clearInterval(check);agent.destroy();error?reject(error):resolve();};
        ...
        });request.setTimeout(120000,()=>request.destroy(fail(504,'GPU output transfer timed out.')));request.on('error',finish);
