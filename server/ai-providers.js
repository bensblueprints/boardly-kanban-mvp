...
async function boundedJSON(response){
 ...
 if(!response.ok){
  ...
  throw fail(response.status>=400&&response.status<500?response.status:502,'The AI provider could not complete the request. Check the model, connection and provider dashboard.');
 }
 return value;
}
function createAIProviders({db,key,tailnet,request=fetch,account,setMode}){
 ...
 return {router,publicState,activeState,authorize,respond,save,activate,fallbackAuthorize,fallbackForRun,startFallback,setFallback,fallbackEnabled};
}
