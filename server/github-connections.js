...
function createGithubConnections({ db, key, namespace, request = githubRequest }) {
  ...
  async function run({projectId,companyId,connectionId,action,data={},valid=()=>true,ssh,actor}) {
    ...
    const server = ssh.forJob(projectId,companyId,data.ssh_connection_id,actor);
