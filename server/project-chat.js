...
function createProjectChat({ db, connections, userId, uploadsDir, environment, payments, email, ssh, github }) {
  ...
  router.post('/api/worker/jobs/:id/github/:connectionId/:action',activeJob,express.json({limit:'8mb'}),async(req,res,next)=>{
    try {
      ...
      const valid=()=>{const current=job(req.params.id);return current?.status==='running'&&current.worker_id===req.boardlyConnection.id;};
      const result=await github.run({projectId:req.projectThread.board_id,companyId:req.projectJob.company_id,connectionId:req.params.connectionId,action:req.params.action,data:req.body,valid,ssh});
  ...
  router.post('/api/worker/jobs/:id/ssh/:connectionId',activeJob,(req,res)=>{
    ...
    try{res.setHeader('cache-control','no-store');res.json(ssh.forJob(req.projectThread.board_id,req.projectJob.company_id,req.params.connectionId));}catch(e){res.status(e.status||500).json({error:e.status?e.message:'SSH unavailable'});}
  });
