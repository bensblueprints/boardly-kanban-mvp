...
export default function CloudConnections({ initialTab = 'mcp', onClose }) {
  ...
  return <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-3" onClick={onClose}>
    <section role="dialog" aria-modal="true" aria-label="boredly connections and downloads" onClick={e => e.stopPropagation()} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl">
      ...
      <div className="p-5 space-y-5">
        ...
        {tab === 'downloads' ? <>
          <div><h3 className="font-semibold">boredly for your desktop</h3><p className="text-sm text-zinc-400 mt-1">Keep a local copy of your boards and sync with your cloud workspace.</p></div>
          ...
          <div><h3 className="font-semibold">{tab === 'mcp' ? 'Let your AI client work with boredly' : 'Sync this account with your desktop'}</h3>
          ...
          {tab === 'sync' && <ol className="list-decimal ml-5 space-y-2 text-sm text-zinc-400"><li><button className="text-indigo-300 underline" onClick={() => setTab('downloads')}>Download and open boredly desktop.</button></li><li>Open Sync, then the existing-token form.</li><li>Replace the server URL with the address above, paste your desktop key and connect.</li><li>Click Sync now, then return to your boards. Existing local boards are merged by their identifiers.</li></ol>}
