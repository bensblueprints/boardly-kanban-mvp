...
export default function ComputerUseAssignment({kind='projects',id}){
 ...
 <button className={button+' bg-indigo-600'} disabled={busy||ids.some(id=>!rentals.some(r=>r.id===id&&r.state==='active'))}>Save assignment</button></form>}
 <div className="flex flex-wrap gap-3"><button disabled={busy} className={button} onClick={()=>act(async()=>{await api.del(base);setNotice(kind==='projects'?'Project override removed. The company assignment applies.':'Company assignment removed.');})}>{kind==='projects'?'Use company assignment':'Remove company assignment'}</button>{kind==='projects'&&<button disabled={busy} className={button} onClick={()=>act(async()=>{await api.put(base,{rental_ids:[],allow_agent:false});setNotice('Computer use disabled for this project.');})}>Disable for this project</button>}</div>
 </>}</>}
 <p className="text-xs text-zinc-400">Desktop control is awaiting the worker desktop service. The same rental shared across projects uses the same desktop; use separate computers when work needs separate storage and sessions.</p>
 <p className="text-sm text-zinc-400">8 GB Standard · $24.99 / 30 days<br/>16 GB Creator · $39.99 / 30 days</p><a href="https://computeruse.space/" target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-300 underline">Check ComputerUse availability</a></section>;
}
