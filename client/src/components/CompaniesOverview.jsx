...
export default function CompaniesOverview({onOpen,onCompany}){
 ...
 return <section aria-label="All companies agent graph" data-activity-motion={motion?'on':'off'} className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4 sm:p-6 space-y-5">
  <style>{`@keyframes overview-flow{to{stroke-dashoffset:-36}}.overview-flow{stroke-dasharray:6 12;animation:overview-flow 1.4s linear infinite}[data-activity-motion="off"] .overview-flow{animation:none}`}</style>
  <header className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-semibold text-lg flex items-center gap-2"><Network size={20} className="text-indigo-300"/>Your companies, live</h2><p className="text-sm text-zinc-400 mt-1">See every project agent and which companies need attention.</p></div><div className="flex flex-wrap items-center gap-2"><ActivityMotion enabled={motion} onChange={setMotion}/><button aria-label="Refresh company graph" className="p-2 rounded-lg border border-zinc-700 hover:bg-zinc-800" onClick={()=>setRetry(n=>n+1)}><RefreshCw size={16}/></button></div></header>
