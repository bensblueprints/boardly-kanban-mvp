...
function ScopeChooser({connector,onCancel}){
 ...
 const companyOnly=['emails','discord','telegram'].includes(connector),projects=data?.projects.filter(p=>data.boards.some(b=>b.id===p.parent_board_id&&String(b.company_id)===company))||[];
 return <section aria-label="Choose connector scope" className="rounded-xl border border-indigo-500/40 p-5 space-y-4"><h3 ref={heading} tabIndex={-1} className="font-semibold outline-none">Where should this connection be used?</h3><p className="text-sm text-zinc-400">{companyOnly?(connector==='emails'?'A company inbox is shared with its projects.':'This bot will send updates for the company you choose.'):'Environment secrets and purchase cards belong to an individual project.'}</p>{error&&<p role="alert" className="text-rose-300">{error}</p>}{!data&&!error&&<p className="text-sm text-zinc-400">Loading your companies…</p>}{data&&<><label className="block text-sm">Company<select aria-label="Connection company" value={company} onChange={e=>{setCompany(e.target.value);setProject('');}} className={settingsInput+' mt-2'}><option value="">Choose a company</option>{data.companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>{!companyOnly&&<label className="block text-sm">Project<select aria-label="Connection project" value={project} onChange={e=>setProject(e.target.value)} className={settingsInput+' mt-2'} disabled={!company}><option value="">Choose a project</option>{projects.map(p=><option key={p.id} value={p.id}>{data.boards.find(b=>b.id===p.parent_board_id)?.name} / {p.name}</option>)}</select></label>}<button className={settingsButton+' bg-indigo-600'} disabled={companyOnly?!company:!project} onClick={()=>location.hash=companyOnly?`#/company/${company}/settings/${connector}`:`#/board/${project}/settings/${connector}`}>Open {companyOnly?'company':'project'} settings</button>{!data.companies.length&&<p className="text-sm text-zinc-400">Create a company on Companies home first.</p>}</>}<button className={settingsButton+' ml-2'} onClick={onCancel}>Cancel</button></section>;
}
