...
export default function AccountComputerUse({focus=false}){
 ...
 <p className="text-sm text-zinc-400">Connect once, then assign one or more computers, including Free Desktops, in Company → Computers or Project → Computer use. A company assignment is inherited by its projects.</p>
 ...
 <form className="space-y-3" onSubmit={e=>{e.preventDefault();act(async()=>{await api.put(base,{token});setToken('');setRentals(null);setNotice('Account verified. Assign your active computers to a company or project.');});}}>
 ...
 <p className="text-xs text-zinc-400">Create a key with account:read, desktop:read and desktop:write in ComputerUse. It is encrypted on the server and excluded from agent prompts. {state.saved&&'Replacing the key clears current computer assignments.'}</p>
 ...
 {state.saved&&<div className="flex flex-wrap gap-3"><button disabled={busy} className={button} onClick={()=>act(async()=>{setRentals((await api.get(base+'/rentals')).rentals);})}>Check my computers</button><button disabled={busy} className={button} onClick={()=>{if(window.confirm('Disconnect ComputerUse and clear its company and project assignments? Your rentals and billing at ComputerUse will continue.'))act(async()=>{await api.del(base);setToken('');setRentals(null);setNotice('Boardly connection and assignments removed.');});}}>Disconnect</button></div>}
 {rentals&&<div className="text-sm text-zinc-400">{rentals.length?rentals.map(r=><p key={r.id}>{r.label||(r.plan==='creator'?'16 GB Creator':'8 GB Standard')} · {r.kind==='pilot'?'Free Desktop':r.plan==='creator'?'16 GB Creator':'8 GB Standard'} · {r.state}{r.available===false?' · offline':''}</p>):'No computers yet. Free Desktops and active rentals appear here.'}</div>}</>}
 ...
 <p className="text-xs text-zinc-400">8 GB Standard: $24.99 / 30 days. 16 GB Creator: $39.99 / 30 days. Per computer, subject to availability; AI usage is separate. Work agents can view and control assigned desktops. Take control in ComputerUse to pause agents; hand back control to resume them.</p></section>;
}
