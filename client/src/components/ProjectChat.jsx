...
export default function ProjectChat({ board, task = null, onClose, onUpdated, initialThreadId = null }) {
  ...
  return <aside role="dialog" aria-label={`Codex chat for ${board.name}`} aria-modal="false" className="coding-pane h-full min-h-0 min-w-0 flex flex-col overflow-y-auto bg-zinc-950">
    ...
    {context&&<div aria-label="Saved GitHub connection" className="shrink-0 px-4 py-3 text-xs border-b border-zinc-800 space-y-1"><p className="text-zinc-300 break-words">{context.github.saved?`GitHub: ${context.github.repository} · ${context.github.branch}`:context.github.status==='restricted'?'GitHub access is managed by the company owner.':'No GitHub connection saved for this project.'}</p>{context.github.saved&&<p className="text-zinc-500">{context.github.inherited?'Inherited from company':'Saved to project'} · Available across this project’s chats{!context.github.allow_agent?' · Agent access paused':''}</p>}{context.can_manage_github&&<button aria-expanded={githubOpen} onClick={()=>setGithubOpen(!githubOpen)} className="text-indigo-300 underline">{githubOpen?'Close GitHub settings':context.github.saved?'GitHub settings':'Connect GitHub'}</button>}</div>}
