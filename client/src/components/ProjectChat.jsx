...
import ProjectEmployees from './ProjectEmployees.jsx';
...
import {ComputerWindowButton} from './LiveComputerWindow.jsx';

export default function ProjectChat({ board, task = null, onClose, onUpdated, initialThreadId = null, onConversationChange }) {
  ...
  const selectConversation = thread => { setSelected(thread?.id||'');onConversationChange?.(thread||null); };
  const startConversation = () => { selectConversation(null); setConversation(null); setMode('ask'); setError(''); };
  useEffect(() => {
    if (!maximized) return;
    ...
    for (let node = pane.current; node?.parentElement; node = node.parentElement) {
      for (const sibling of node.parentElement.children) {
        // Native dialogs manage background inertness when opened from this chat.
        if (sibling !== node && sibling instanceof HTMLElement && !(sibling instanceof HTMLDialogElement)) {
  ...
  useEffect(() => {
    let active = true;
    api.get(threadsUrl).then(async t => { let chosen=initialThreadId===''?null:t.find(x=>x.id===initialThreadId)||t[0]||null;if(initialThreadId&&!t.some(x=>x.id===initialThreadId)){try{const data=await api.get('/api/chat/threads/'+encodeURIComponent(initialThreadId));if(data.thread.board_id===board.id&&(!task||data.thread.card_id===task.id)){chosen=data.thread;t=[chosen,...t];}}catch(e){if(![403,404].includes(e.status))throw e;}}if (active) { setThreads(t); selectConversation(chosen); } }).catch(e => { if (active) setError(e.message); });
    return () => { active = false; };
  ...
  const newThread = async () => {
    ...
    await loadThreads(); selectConversation(t); return t.id;
  };
  const send = async e => {
  ...
  return <aside ref={pane} role="dialog" aria-label={`Codex chat for ${board.name}`} aria-modal={maximized} onKeyDown={handleWindowKey} className={`coding-pane ${maximized ? 'coding-pane-maximized' : ''} h-full min-h-0 min-w-0 flex flex-col overflow-y-auto bg-zinc-950`}>
    ...
    <div className="shrink-0 flex flex-wrap items-center gap-2 px-4 py-3 border-b border-zinc-800"><button disabled={readOnly} title={readOnly?'Audio requires Editor access to this project':'Hear priorities, blockers and next steps'} className="flex items-center gap-2 rounded-lg border border-indigo-400 px-3 py-2 text-sm text-indigo-100 disabled:opacity-40" onClick={()=>setAudioOpen(true)}><Headphones size={17}/>Audio briefing</button><ComputerWindowButton/>{!readOnly&&<button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm" onClick={()=>setTeamOpen(v=>!v)}>Employees</button>}</div>
    {audioOpen&&<AudioBriefing kind="project" id={board.id} onClose={()=>setAudioOpen(false)}/>}
    <div className="shrink-0 p-3 flex gap-2 border-b border-zinc-800"><select aria-label="Conversation history" disabled={busy} value={selected} onChange={e => { if (!e.target.value) startConversation(); else { selectConversation(threads.find(t=>t.id===e.target.value)); setError(''); } }} className="min-w-0 flex-1 rounded-lg bg-zinc-900 border border-zinc-700 px-2 py-2 text-sm"><option value="">New conversation</option>{threads.map(t => <option key={t.id} value={t.id}>{t.title}{['queued','running','recovering','blocked'].includes(t.job_status)?` · ${t.job_mode==='work'?'Work':t.job_mode==='plan'?'Plan':'Ask'} ${t.job_status}`:''}</option>)}</select>{!readOnly&&<button type="button" aria-label="Start new conversation" title="New chat in Ask mode" disabled={busy} onClick={startConversation} className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-zinc-800 rounded-lg text-sm disabled:opacity-40"><Plus size={18} />New chat</button>}</div>
    ...
    {teamOpen?<ProjectEmployees board={board} onClose={()=>setTeamOpen(false)}/>:sshOpen&&context?.can_manage_ssh?<div className="flex-1 min-h-36 overflow-y-auto p-4"><SshConnections key={board.id} kind="projects" id={board.id}/></div>:githubOpen&&context?.can_manage_github?<div className="flex-1 min-h-36 overflow-y-auto p-4"><GithubConnection key={board.id} kind="projects" id={board.id}/></div>:<>
    ...
    <form onSubmit={send} className="chat-composer shrink-0 border-t border-zinc-800 space-y-3">
      ...
      <div className="flex flex-wrap justify-end items-center gap-x-4 gap-y-2"><span className="mr-auto text-xs text-zinc-600">Ctrl / ⌘ + Enter to send</span>{running&&!readOnly ? <button type="button" onClick={async () => { try { await api.post(`/api/chat/jobs/${conversation.job.id}/cancel`, {}); setConversation(await api.get(`/api/chat/threads/${selected}`)); } catch (e) { setError(e.message); } }} className="flex shrink-0 items-center gap-2 whitespace-nowrap text-sm px-3 py-2 rounded-lg bg-zinc-800"><Square size={14} /> Stop</button> : <button disabled={!mode || !input.trim() || busy || readOnly || (personal&&!online)} className="flex shrink-0 items-center gap-2 whitespace-nowrap text-sm px-4 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg disabled:opacity-40"><Send size={15} />{busy ? 'Sending…' : mode==='work'?'Start work':mode==='plan'?'Discuss plan':'Ask AI'}</button>}</div>
    </form>
    </>}
  </aside>;
}
