...
(async()=>{
 ...
 await owner.goto(url+`#/company/${a.company.id}/settings/members`);await owner.getByRole('button',{name:'Set up later',exact:true}).click();
 ...
 assert.equal(await row.getByRole('checkbox').count(),7);for(const box of await row.getByRole('checkbox').all())assert.equal(await box.isChecked(),false);
 await projectRow.getByRole('button',{name:'Permission scopes',exact:true}).click();assert.equal(await projectRow.getByRole('checkbox').count(),7);
 await member.goto(url+`?user=${user}#/board/${a.project.id}`);await member.getByRole('button',{name:'Set up later',exact:true}).click();await member.getByLabel('Project name',{exact:true}).waitFor();
 ...
 await toggle('Environment');await member.goto(url+`?user=${user}#/board/${a.project.id}/settings/environment`);await member.getByLabel('Variable name',{exact:true}).waitFor();
 await toggle('Environment',false);await member.getByRole('heading',{name:'Setting unavailable',exact:true}).waitFor({timeout:6500});
 ...
 await member.goto(url+`?user=${user}#/board/${a.project.id}/settings/payments`);
 ...
 await member.goto(url+`?user=${user}#/board/${a.project.id}/settings/members`);
 await member.getByRole('region',{name:'Members',exact:true}).waitFor();assert.equal(await member.getByRole('button',{name:'Permission scopes',exact:true}).count(),0);
 ...
 await member.goto(url+`?user=${user}#/company/${a.company.id}/settings/connectors`);await member.getByRole('button',{name:'SSH computers & servers',exact:true}).waitFor();await member.getByRole('button',{name:'GitHub',exact:true}).waitFor();
 await toggle('Environment',true);await owner.screenshot({path:'/tmp/boardly-settings-scopes-owner.png'});
 await owner.setViewportSize({width:390,height:844});await owner.screenshot({path:'/tmp/boardly-settings-scopes-mobile.png'});
 ...
 const guest=await browser.newPage();guest.on('pageerror',error=>errors.push(error.message));await guest.goto(url+`?user=${projectAdded.member.user_id}#/board/${a.project.id}/settings/connectors`);await guest.getByRole('button',{name:'Set up later',exact:true}).click();await guest.getByRole('button',{name:'GitHub',exact:true}).waitFor();assert.equal(await guest.getByRole('button',{name:'SSH',exact:true}).count(),0);
