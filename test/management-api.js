...
(async () => {
  ...
  try {
    ...
    const transferTool=async(name,args)=>{const r=await rpc(name,args,key);assert.ok(!r.result?.isError,r.result?.content?.[0]?.text);return JSON.parse(r.result.content[0].text);};
    const payload=Buffer.alloc(3*1024*1024,42),upload=await transferTool('start_file_upload',{board_id:a.project.id,name:'large-binary.dat',size:payload.length});
    for(let offset=0;offset<payload.length;offset+=1024*1024)await transferTool('upload_file_chunk',{board_id:a.project.id,upload_id:upload.upload_id,offset,content_base64:payload.subarray(offset,offset+1024*1024).toString('base64')});
    check((await transferTool('file_upload_status',{board_id:a.project.id,upload_id:upload.upload_id})).offset===payload.length,'MCP transfer exceeds the 2 MB inline limit using bounded chunks');
    const saved=await transferTool('finish_file_upload',{board_id:a.project.id,upload_id:upload.upload_id});
    check((await transferTool('finish_file_upload',{board_id:a.project.id,upload_id:upload.upload_id})).id===saved.id,'MCP finalization is idempotent');
    await transferTool('cancel_file_upload',{board_id:a.project.id,upload_id:upload.upload_id});
    check((await f.request('/api/project-files/'+saved.id+'/download')).status===200,'Completed file survives cancelling a finished transfer');
    await f.api('/api/project-files/'+saved.id,{method:'DELETE'});
