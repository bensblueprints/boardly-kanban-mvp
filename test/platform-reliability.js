...
(async()=>{
 ...
 try{
  ...
  const reassigned=await f.api(`/api/boards/${p.project.id}/employees/assign`,{method:'POST',body:{employee_id:final.employees[0].id,card_id:card.id,instruction:'Review this completed task again.'}});await until(async()=>{const r=await f.api(`/api/chat/threads/${reassigned.thread_id}`);return r.job.status==='completed';});assert.equal((await f.api(`/api/cards/${card.id}`)).list_id,completedCard.list_id,'explicit reassignment reopens and completes the same task');
