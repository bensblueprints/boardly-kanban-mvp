...
(async()=>{
 ...
 const actionJobs=new Map();let copies=0;const actions={projects:()=>[{id:7,name:'Client'}],saveOutput:async()=>{copies++;return{id:99,name:'result.png',mime:'image/png',size:100};},start:args=>{if(!actionJobs.has(args.id))actionJobs.set(args.id,{...args,thread_id:'thread-1',status:'queued'});return actionJobs.get(args.id);},status:id=>actionJobs.get(id),cancel:id=>{actionJobs.get(id).status='cancelled';}};
 ...
 const make=()=>createGpuWorkflows({db,ssh,ownerId:'owner',generate,remote,actions:()=>actions,interval:3600000});let gpu=make();
 ...
 try {
  ...
  pendingText=new Promise(r=>releaseText=r);const editId=crypto.randomUUID();gpu.start(wf.id,{request_id:editId,prompt:'scene'});await wait(20);
  const editRoute='/api/gpu/runs/'+editId+'/steps/1';
  assert.equal((await request(editRoute,{previous_prompt:'wrong',prompt:'scene revised'},'PATCH')).status,409);
  assert.equal((await request(editRoute,{previous_prompt:'{{previous_text}}',prompt:'scene revised'},'PATCH')).status,200);
  releaseText({output_text:'scene first'});await wait(20);pendingText=null;
  assert.equal(gpu.run(editId).steps[1].prompt,'scene revised','An in-flight previous step cannot overwrite a future edit');
  await gpu.tick();assert.equal(submitted.get(gpu.run(editId).steps[1].id)['1'].inputs.text,'scene revised');
  assert.equal((await request(editRoute,{previous_prompt:'scene revised',prompt:'changed too late'},'PATCH')).status,409);
  await gpu.tick();await gpu.tick();
  assert.throws(()=>gpu.saveWorkflow({name:'Wrong project',steps:[{kind:'email',project_id:8,prompt:'Email file'}]}),/Choose the project/);
  const delivery=gpu.saveWorkflow({name:'Image then email',steps:[{kind:'image',template_id:template.id,prompt:'scene {{prompt}}'},{kind:'email',project_id:7,prompt:'Email this to client@example.test with {{previous_text}}'}]});
  const deliveryId=crypto.randomUUID();gpu.start(delivery.id,{request_id:deliveryId,prompt:'scene'},'codex');await wait(20);await gpu.tick();await gpu.tick();
  const actionId=gpu.run(deliveryId).steps[1].id;assert.equal(actionJobs.size,1);assert.equal(copies,1);assert.equal(actionJobs.get(actionId).runtime,'codex');assert.equal(actionJobs.get(actionId).context.files[0].id,99);assert.deepEqual(actionJobs.get(actionId).context.files[0].source,{ssh_connection_id:'ssh-a',comfyui_port:8188,filename:'result.png',subfolder:'test',type:'output'});
  gpu.close();gpu=make();await wait(20);assert.equal(actionJobs.size,1);assert.equal(copies,1,'Restart polls the same action without recopying or resending');
  actionJobs.get(actionId).status='blocked';actionJobs.get(actionId).blocker='Connect the client mailbox';await gpu.tick();assert.equal(gpu.run(deliveryId).status,'blocked');assert.match(gpu.run(deliveryId).error,/Connect the client/);
  actionJobs.get(actionId).status='completed';actionJobs.get(actionId).draft='Sent with provider receipt test-123';await request('/api/gpu/runs/'+deliveryId+'/resume',{});await wait(20);assert.equal(gpu.run(deliveryId).status,'completed');
  const cancelled=crypto.randomUUID();gpu.start(delivery.id,{request_id:cancelled,prompt:'scene'});await wait(20);await gpu.tick();await gpu.tick();await request('/api/gpu/runs/'+cancelled+'/cancel',{});assert.equal(actionJobs.get(gpu.run(cancelled).steps[1].id).status,'cancelled');
  const quick=await request('/api/gpu/workers/'+w.id+'/jobs',{request_id:crypto.randomUUID(),template_id:template.id,prompt:'scene quick'});assert.equal(quick.status,202);await wait(20);await gpu.tick();assert.equal(gpu.run(quick.data.id).status,'completed');
  console.log('PASS: queued prompt concurrency, locked submitted prompts, per-GPU jobs, scoped actions, exact output handoff, action restart deduplication, blocked state and cancellation');
