...
(async()=>{
 ...
 try {
  ...
  const actionId=gpu.run(deliveryId).steps[1].id;assert.equal(actionJobs.size,1);assert.equal(copies,1);assert.equal(actionJobs.get(actionId).runtime,'codex');assert.equal(actionJobs.get(actionId).context.files[0].id,99);
