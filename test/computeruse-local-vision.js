...
(async()=>{
 ...
 try{
  ...
  assert.equal((await f.api(base)).mode,'gpt');assert.equal((await f.api(base)).share_with_company,false);
  ...
  async function hostedCycle(user='user_owner',expected=true){
   ...
   const thread=await f.api(`/api/boards/${p.project.id}/chat/threads`,{...opts,method:'POST',body:{}});await f.api(`/api/chat/threads/${thread.id}/messages`,{...opts,method:'POST',body:{mode:'work',content:'Read the screen with the company GPU.'}});
  ...
  await f.api(base+'/test',{method:'POST',body:{connection_id:cid}});
  ...
  await assert.rejects(()=>run('inspect',member),/share GPU/);await cu.releaseRun('fixture-run');
  const grant=(await f.api(`/api/companies/${p.company.id}/members`)).members.find(m=>m.email==='gpu-member@example.com').grant_id;
  await f.api(`/api/memberships/${grant}`,{method:'PATCH',body:{scopes:['computers']}});
  await hostedCycle(member,false); // Company membership alone cannot opt into the owner's GPU.
  await f.api(base,{method:'PUT',body:{mode:'local',connection_id:cid,share_with_company:true}});
  assert.throws(()=>ssh.forService(cid,member),/not assigned/,'inference sharing does not grant SSH');
  await hostedCycle(member,true); // Real cloud membership callback + owner-only SSH + text-only GPT.
  assert.throws(()=>ssh.forService(cid,member),/not assigned/);
  assert.equal((await f.request(base+'/detect',{user:member,workspace:'user_owner',method:'POST',body:{connection_id:cid}})).status,403,'members cannot manage the owner GPU');
  allowedMembers.add(member);assert.equal((await run('inspect',member)).mode,'local');await cu.releaseRun('fixture-run');
  onInspect=()=>allowedMembers.delete(member);await assert.rejects(()=>run('inspect',member),/permission changed|share GPU/);onInspect=()=>{};await cu.releaseRun('fixture-run');
  onInspect=()=>vision.save({mode:'local',connection_id:cid,share_with_company:false});allowedMembers.add(member);await assert.rejects(()=>run('inspect',member),/changed/);onInspect=()=>{};await cu.releaseRun('fixture-run');vision.save({mode:'local',connection_id:cid,share_with_company:true});
  onInspect=()=>f.api(`/api/memberships/${grant}`,{method:'PATCH',body:{scopes:[]}});await hostedCycle(member,false);onInspect=()=>{};
  await hostedCycle(member,false); // Revoked Computer use scope is enforced on the next run too.
  const guest=(await f.api(`/api/projects/${p.project.id}/members`,{method:'POST',body:{email:'gpu-guest@example.com',role:'editor'}})).member.user_id;
  const guestGrant=(await f.api(`/api/companies/${p.company.id}/members`)).project_members.find(m=>m.email==='gpu-guest@example.com').grant_id;
  await f.api(`/api/memberships/${guestGrant}`,{method:'PATCH',body:{scopes:['computers']}});await hostedCycle(guest,false);
  ...
  const second=await f.api(base,{user:'user_second'});assert.equal(second.mode,'gpt');assert.equal(second.share_with_company,false);assert.deepEqual(second.connections,[]);
  const own=await f.api('/api/account/ssh',{user:'user_second',method:'POST',body:{...config,label:'Second account GPU'}});
  await f.api(base+'/test',{user:'user_second',method:'POST',body:{connection_id:own.id}});await f.api(base,{user:'user_second',method:'PUT',body:{mode:'local',connection_id:own.id}});
  assert.equal((await f.api(base,{user:'user_second'})).connection_id,own.id,'new accounts can use their own tested GPU');
  ...
  console.log('PASS: pinned SSH + jump host; company GPU sharing without SSH grants; hosted text-only cycles; sharing/scope revocation during inference; project-guest rejection; native helper no images; reversible modes; offline no fallback; independent new-account GPUs');
