...
(async()=>{
 ...
 await owner.goto(url+`#/board/${a.project.id}`);await owner.getByRole('button',{name:'Set up later',exact:true}).click();await owner.getByRole('button',{name:'Chat with AI',exact:true}).click();
 ...
 const account=owner.getByRole('region',{name:'Account GitHub',exact:true}),patField=account.getByLabel('Account GitHub personal access token',{exact:true}),pat='github_pat_fixture_browser_12345678901234567890';
 await patField.waitFor();
 ...
 await owner.getByRole('button',{name:'Back to workspace',exact:true}).click();await owner.getByRole('button',{name:'Chat with AI',exact:true}).click();await chat.getByRole('button',{name:'GitHub settings',exact:true}).click();
 ...
 await owner.evaluate(()=>window.dispatchEvent(new CustomEvent('boardly-account',{detail:{section:'github'}})));await account.getByText('Account PAT saved · Verified as fixture-owner',{exact:true}).waitFor();assert.equal(await patField.inputValue(),'');
 await owner.setViewportSize({width:390,height:844});await patField.scrollIntoViewIfNeeded();await owner.screenshot({path:'/tmp/boardly-settings-github-mobile.png'});assert.ok(await owner.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await owner.getByRole('button',{name:'Back to workspace',exact:true}).click();await owner.getByRole('button',{name:'Chat with AI',exact:true}).click();
 ...
 await owner.screenshot({path:'/tmp/boardly-settings-github-desktop.png'});
 ...
 await member.goto(url+`?user=${user}#/board/${a.project.id}`);await member.getByRole('button',{name:'Set up later',exact:true}).click();await member.getByRole('button',{name:'Chat with AI',exact:true}).click();await member.getByText('GitHub: company/main · main',{exact:true}).waitFor();
 await member.evaluate(()=>window.dispatchEvent(new CustomEvent('boardly-account',{detail:{section:'billing'}})));await member.getByText('Shared account · Owner',{exact:true}).waitFor();assert.equal(await member.getByLabel('Account GitHub personal access token',{exact:true}).count(),0);
