...
(async () => {
  ...
  await page.screenshot({ path: '/home/ben/.local/share/boardly-ops/shared-companies-visible.png', fullPage: true });
  const sharedBounds=await page.getByRole('button',{name:/Clothing Company/}).boundingBox();
  assert.ok(sharedBounds.y+sharedBounds.height<1000,'Shared company is visible above the fold');
