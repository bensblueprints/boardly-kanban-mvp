...
(async()=>{
 ...
 const f=await fixture({publicAccess:true,providerRequest:async(url,options)=>{
  ...
  assert.ok(context?.content.includes('Shared Hetzner'),'Hosted chat receives durable account SSH context');assert.ok(!JSON.stringify(body).includes(secret));
 ...
 try{
  ...
  for(const mode of ['ask','plan','work']){
   ...
   const {job}=await worker('/api/worker/claim');assert.equal(job.mode,mode);const rows=mode==='work'?job.ssh:job.context[0].ssh.connections;
  ...
  const discussion=(await worker('/api/worker/claim')).job;assert.equal(discussion.context.projects[0].ssh.connections[0].id,connection.id);
  ...
  assert.equal(seen.length,3);console.log('PASS: account SSH metadata in existing/new project Ask Plan Work and company discussions, future company inheritance, pause refresh, native/hosted contexts, secrets and member/workspace isolation');
