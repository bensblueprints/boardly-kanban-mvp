...
async function createComputerUseBroker({socketPath,request,onActivity=()=>{}}){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  let activityKey,activityTitle;
  try{
   if(req.method!=='POST'||!['/list','/status','/screenshot','/inspect','/action','/release','/logins','/login'].includes(req.url))throw Error('Unknown computer action');
   let raw='';for await(const part of req){raw+=part;if(Buffer.byteLength(raw)>25000)throw Error('Request too large');}
   const command=req.url.slice(1),key=activityKey='computer:'+ ++count;activityTitle='Computer · '+command;onActivity(key,activityTitle,'running');
  ...
  }catch(e){if(activityKey)onActivity(activityKey,activityTitle,'failed');res.writeHead(e.status||502);res.end(JSON.stringify({error:e.status?e.message:'Computer operation could not complete. Inspect its status before retrying.'}));}
 });
