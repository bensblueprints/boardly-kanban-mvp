...
function createBoardlyServer({ db, uploadsDir, ownerId: ownerOpt = null }) {
  const server = new McpServer({ name: 'boardly', version: '1.1.0' });

  // ---- helpers (mirrors server/app.js) ----

  // Sync bookkeeping + ownership, mirroring server/app.js: new rows get a
  // UUID, an updated_at stamp, a rev from the store-wide counter, and an
  // owner. On the desktop the owner is the implicit local user; cloud MCP
  // passes the token's user per request. Every entry-point lookup is scoped
  // by owner_id (denormalised onto every syncable row) — on the desktop the
  // local user owns everything, so the result sets are unchanged there.
  const ownerId = ownerOpt ?? (db.localUser ? db.localUser.id : 0);

  const q = {
    board: 'SELECT * FROM boards WHERE id = ? AND owner_id = ?',
    list: 'SELECT * FROM lists WHERE id = ? AND owner_id = ?',
    card: 'SELECT * FROM cards WHERE id = ? AND owner_id = ?',
    label: 'SELECT * FROM labels WHERE id = ? AND owner_id = ?'
  };
  ...
  async function mustGet(sql, id, what) {
    const row = await db.one(sql, [id, ownerId]);
  ...
  tool('list_boards', 'List all boards with list/card counts', {}, async () => {
    const boards = [];
    for (const b of await db.all('SELECT * FROM boards WHERE owner_id = ? ORDER BY starred DESC, id DESC', [ownerId])) {
  ...
  tool('update_board', 'Update a board (rename, set the project description, recolor, star/unstar)', {
    ...
    return db.one(q.board, [board.id, ownerId]);
  });
  ...
  tool('create_list', 'Add a list (column) to a board', {
    ...
    const list = await db.one(
      `INSERT INTO lists (board_id, name, position, uid, owner_id, updated_at, rev)
       VALUES (?, ?, ?, ?, ?, {{now}}, ?) RETURNING *`,
      [board.id, name.trim(), pos, crypto.randomUUID(), ownerId, await nextRev()]
    );
  ...
  tool('update_list', 'Rename or archive/restore a list', {
    ...
    return db.one(q.list, [list.id, ownerId]);
  });
    ...
    const rows = board_id === undefined
      ? await db.all(
          `SELECT c.*, l.name AS list_name, l.board_id, b.name AS board_name
           ...
           WHERE c.archived = 0 AND c.title {{ilike}} ? AND c.owner_id = ?
           ORDER BY c.id LIMIT ?`,
          [`%${query}%`, ownerId, limit || 50]
        )
      : await db.all(
          `SELECT c.*, l.name AS list_name, l.board_id, b.name AS board_name
           FROM cards c
           JOIN lists l ON l.id = c.list_id
           JOIN boards b ON b.id = l.board_id
           WHERE c.archived = 0 AND c.title {{ilike}} ? AND c.owner_id = ? AND l.board_id = ?
           ORDER BY c.id LIMIT ?`,
          [`%${query}%`, ownerId, board_id, limit || 50]
        );
    const out = [];
  ...
  tool('create_card', 'Add a card to a list', {
    ...
    const card = await db.one(
      ...
      [list.id, title.trim(), description || '', due_date || null, pos,
       crypto.randomUUID(), ownerId, await nextRev()]
    );
  ...
  tool('update_card', 'Update a card (title, description, due date, archive/restore)', {
    ...
    return cardSummary(await db.one(q.card, [card.id, ownerId]));
  });
  ...
  tool('create_label', 'Create a label on a board', {
    ...
    return db.one(
      `INSERT INTO labels (board_id, name, color, uid, owner_id, updated_at, rev)
       VALUES (?, ?, ?, ?, ?, {{now}}, ?) RETURNING *`,
      [board.id, name.trim(), color || '#22c55e', crypto.randomUUID(), ownerId, await nextRev()]
    );
  });
  ...
  tool('assign_label', 'Attach a label to a card', {
    ...
    await db.run('INSERT OR IGNORE INTO card_labels (card_id, label_id, uid, owner_id, updated_at, rev) VALUES (?, ?, ?, ?, {{now}}, ?)',
      [card.id, label.id, crypto.randomUUID(), ownerId, await nextRev()]);
  ...
  tool('add_checklist', 'Add a checklist to a card', {
    ...
    const row = await db.one(
      ...
      [card.id, clTitle, pos, crypto.randomUUID(), ownerId, await nextRev()]
    );
  ...
  tool('add_checklist_item', 'Add an item to a checklist', {
    ...
    const cl = await mustGet('SELECT * FROM checklists WHERE id = ? AND owner_id = ?', checklist_id, 'Checklist');
    ...
    return db.one(
      `INSERT INTO checklist_items (checklist_id, text, position, uid, owner_id, updated_at, rev)
       VALUES (?, ?, ?, ?, ?, {{now}}, ?) RETURNING *`,
      [cl.id, text.trim(), pos, crypto.randomUUID(), ownerId, await nextRev()]
    );
  });
  ...
  tool('set_checklist_item', 'Update a checklist item (edit text, mark done/undone)', {
    ...
    const item = await mustGet('SELECT * FROM checklist_items WHERE id = ? AND owner_id = ?', item_id, 'Checklist item');
  ...
  tool('delete_attachment', 'Remove an attachment from a card', {
    ...
    const row = await mustGet('SELECT * FROM attachments WHERE id = ? AND owner_id = ?', attachment_id, 'Attachment');
  ...
  tool('add_comment', 'Add a comment to a card', {
    ...
    const comment = await db.one(
      `INSERT INTO comments (card_id, author, body, uid, owner_id, updated_at, rev)
       VALUES (?, ?, ?, ?, ?, {{now}}, ?) RETURNING *`,
      [card.id, name, body.trim(), crypto.randomUUID(), ownerId, await nextRev()]
    );
